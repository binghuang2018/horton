...
class ODASCFSolver(object):
    ...
    def __call__(self, ham, lf, overlap, occ_model, *dm0s):
        ...
        fock0s = [lf.create_two_index() for i in range(ham.ndm)]
        fock1s = [lf.create_two_index() for i in range(ham.ndm)]
        dm1s = [lf.create_two_index() for i in range(ham.ndm)]
        exps = [lf.create_expansion() for i in range(ham.ndm)]
        work = dm0s[0].new()
