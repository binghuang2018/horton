...
class IOData(object):
    ...
    def __init__(self, **kwargs):
        for key in kwargs: # key, value in kwargs.iteritems():
            value = kwargs[key]
    ...
    def from_file(cls, *filenames, **kwargs):
        ...
        if len(kwargs) > 0:
            raise TypeError('Keyword argument(s) not supported: %s' % list(kwargs.keys()))
        ...
        if 'permutation' in result:
            for key in result: #, value in result.iteritems():
                value = result[key]
